// Everything can be represented by numbers, making this one of the most important packages. Contains functionalities such as:
//   - Formatting numbers into text.
//   - Numbers interacting with ranges.
//   - Transforming, wrapping, mapping, limiting, snapping numbers.
...
func Indexes2DToIndex1D[T Integer](x, y, columns, rows T) T {
	var result = x*columns + y
...
func Index1DToIndexes2D[T Integer](index, columns, rows T) (x, y T) {
	var max = columns * rows
	if index < 0 {
		index = 0
	} else if index > max {
